...
$colis=$db->prepare("SELECT  * FROM colis,transfert,agence WHERE colis.id_transfert=transfert.id_transfert AND agence.id_agence=colis.id_agence  AND agence.id_agence=? ");
...
$trans=$db->prepare("SELECT  * FROM transfert,user,agence WHERE agence.id_agence=user.id_agence AND transfert.id_user=user.id_user AND agence.id_agence=? AND DATE(transfert.create_at) = CURDATE()  ");
...
if(isset($_POST['valider']))
{
  ...
  if(control_champ(["caracteristique","type","nom1","nom2","prenom2","prenom1","mail2","mail1","telephone2","telephone1","cni2","cni1"]))
  {
   ...
   $slach='-';
   $reference = strtoupper($type).$slach . $cni1 .$slach . $cni2;
        ...
        ************/
        ...
        if(count($erreur) == 0)
        {
            ...
            $insert->execute([$reference, $caracteristique,$type,$nom1,$agences,$nom2,$prenom2,$prenom1,$mail2,$mail1,$telephone2,$telephone1,$cni2,$cni1,$transfert]);
